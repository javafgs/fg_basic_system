...
$(function () {
    ...
    $.ajax({
        url: ctp+'js/app/admin/menu/materialdesignicons.json',
        type: 'GET',
        dataType: 'json'
...
function initTreeTable() {
    ...
    $FG.initTreeTable('menuTable', setting);
}
...
function refresh() {
    initTreeTable();
    $FG.refreshJsTree("menuTree", createMenuTree());
}
...
function deleteMenus() {
    ...
    $FG.confirm({
        ...
        $.post(ctp + 'admin/menu/deleteBatch', {"ids": ids_arr}, function (r) {
            if (r.code === 0) {
                $FG.n_success(r.msg);
                refresh();
            } else {
...
function exportMenuExcel() {
    $.post(ctp + "admin/menu/excel", $(".menu-table-form").serialize(), function (r) {
        if (r.code === 0) {
...
function exportMenuCsv() {
    $.post(ctp + "admin/menu/csv", $(".menu-table-form").serialize(), function (r) {
        if (r.code === 0) {
            window.location.href = "file/download?fileName=" + r.msg + "&delete=" + true;
        } else {
            $FG.n_warning(r.msg);
