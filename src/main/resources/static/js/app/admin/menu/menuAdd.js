...
$(function () {
    ...
    $("#menu-add .btn-save").click(function () {
        ...
        if (flag) {
            if (name === "save") {
                $FG.sendRequestAsync(ctp + '/admin/menu/add',"post",$menuAddForm.serialize())
            }
            if (name === "update") {
                $FG.sendRequestAsync(ctp + '/admin/menu/update',"post",$menuAddForm.serialize())
            }
        }
    });
...
function closeModal() {
    ...
    $FG.closeAndRestModal("menu-add");
    $FG.refreshJsTree("menuTree", createMenuTree());

}

function createMenuTree() {
    $.post(ctp + "admin/menu/tree", {}, function (r) {
        if (r.code === 0) {
            var data = r.msg;
            $('#menuTree').jstree({
                "core": {
